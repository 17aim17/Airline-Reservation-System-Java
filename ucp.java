...
public class ucp extends JInternalFrame implements ActionListener{
...
ucp(String uname){
  ...
  setSize(1366,712);
  pn =new JPanel();
  lb4 =new JLabel("User Chage Password");
...
public void actionPerformed(ActionEvent e)
{
      try
      {
          ...
          {
              // verify the password
              String verify ="SELECT * FROM employee WHERE user_id ='"+user+"' AND password ='"+op1.getText()+"'";
              ...
              if(rs.next())
              {
                if(np1.getText().matches("(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{8,}")){
                  if(np1.getText().equals(np2.getText()))
                  {
                      ...
                      {
                          String update ="UPDATE employee set password='"+np1.getText()+"' WHERE user_id ='"+user+"'";
      ...
      {
          JOptionPane.showMessageDialog(this,ex+"Sorry , cannot change password at this moment");
      }
}
